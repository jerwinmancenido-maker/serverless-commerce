/**
 * @file    apps/backend/src/workflows/steps/prepare-admin-manual-payment-proof.ts
 * @module  ManualPaymentModule (Workflows)
 * @purpose Validate admin-uploaded payment proof metadata and checksums before file upload.
 * @contracts
 *   Step: prepareAdminManualPaymentProofStep
 */
...
export type AdminManualPaymentProofInput = {
  orderId: string
  actorId: string
  file: {
    fileName: string
...
export type PreparedAdminManualPaymentProof = {
...
export const prepareAdminManualPaymentProofStep = createStep(
  "prepare-admin-manual-payment-proof",
  async (
    input: AdminManualPaymentProofInput,
    { container },
  ): Promise<StepResponse<PreparedAdminManualPaymentProof>> => {
    ...
    if (!paymentSession) {
      throw new MedusaError(
        ...
        "order does not have an active Manual QR payment session",
      )
    }
    ...
    return new StepResponse({
      ...
      submission: {
        ...
        orderId: order.id,
        customerId: order.customer_id || "guest",
...
export default prepareAdminManualPaymentProofStep
