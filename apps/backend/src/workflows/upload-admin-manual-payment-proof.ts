...
import {
  type AdminManualPaymentProofInput,
  prepareAdminManualPaymentProofStep,
} from "./steps/prepare-admin-manual-payment-proof"
import {
  upsertManualPaymentProofStep,
  validateManualPaymentProofSubmissionStep,
} from "./steps/submit-manual-payment-proof"

export const uploadAdminManualPaymentProofWorkflow = createWorkflow(
  "upload-admin-manual-payment-proof",
  function (input: AdminManualPaymentProofInput) {
    const prepared = prepareAdminManualPaymentProofStep(input)
    const uploadInput = transform({ prepared }, ({ prepared }) => ({
      files: [prepared.upload],
    }))
    const changed = when(
      "upload-new-admin-manual-payment-proof",
      { prepared },
...
export default uploadAdminManualPaymentProofWorkflow
