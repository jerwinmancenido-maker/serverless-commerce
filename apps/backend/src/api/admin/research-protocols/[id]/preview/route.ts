...
export async function GET(req: AuthenticatedMedusaRequest, res: MedusaResponse) {
  const { revision_id: revisionId, access_level: accessLevel } = req.validatedQuery as AdminPreviewResearchProtocol
  ...
  const projectedContent = buildResearchProtocolContentForAccess(
    content,
    policy,
    accessLevel,
  )
