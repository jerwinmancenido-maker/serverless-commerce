...
import type { StoreRestoreResearchRoutineLogType } from "../../../validators"
import { createResearchWorkflowContext, getResearchIdempotencyKey, setResearchPrivateNoStore } from "../../../utils"

export async function POST(req: AuthenticatedMedusaRequest<StoreRestoreResearchRoutineLogType>, res: MedusaResponse) {
  ...
  const { result } = await mutateResearchRoutineLogWorkflow(req.scope).run({
    input: { customerId, activeConsentVersion: configuration.activeConsentVersion, logId: req.params.id, operation: "restore", supplyId: req.validatedBody.supply_id, confirmedQuantityBaseUnits: req.validatedBody.confirmed_quantity_base_units, baseUnit: req.validatedBody.base_unit, idempotencyKey, previewToken: req.validatedBody.preview_token },
    context: createResearchWorkflowContext(customerId, "routine-log-restore", idempotencyKey),
  })
  res.json(result)
}
