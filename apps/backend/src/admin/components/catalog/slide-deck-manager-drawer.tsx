...
export const SlideDeckManagerDrawer: React.FC<SlideDeckManagerDrawerProps> = ({
  ...
  const addSlide = () => {
    ...
    setSlides((prev) => [
      ...prev,
      {
        ...
        desc: "Custom reference standard asset render",
      },
    ])
  }
  ...
  return (
    <Drawer open={open} onOpenChange={onOpenChange}>
      <Drawer.Content className="right-0 top-0 bottom-0 h-dvh sm:h-full w-full sm:max-w-xl bg-white border-l border-slate-200 p-0 flex flex-col justify-between shadow-2xl">
        <Drawer.Header className="px-4 sm:px-6 py-3.5 sm:py-5 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <Drawer.Description className="text-xs text-slate-500 mt-0.5">
                {title} ({handle}) &bull; Manage slides, asset paths, and analytical annotations.
              </Drawer.Description>
            </div>
          </div>
        </Drawer.Header>
