  ...
  const createMutation = useMutation({
    mutationFn: async (
      ...
      const variants = preview.matrix.rows.map((row) => {
        ...
        if (!draft?.sku.trim()) {
          throw new Error(`SKU is required for ${row.title}`)
        }
              ...
              return (
                ...
                >
                  ...
                  <div className="grid gap-3 lg:grid-cols-3">
                    <div className="flex flex-col gap-y-2">
                      <Label htmlFor={`sku-${row.key}`}>SKU *</Label>
                      <Input
                        id={`sku-${row.key}`}
                        value={draft?.sku || ""}
                        onChange={(event) =>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-lg border border-ui-border-base p-4">
              ...
              {draftSaveBlockers.length ? (
                ...
                <Text size="small" className="mt-2 text-ui-fg-success">
                  The current payload is ready to save as a native Medusa draft.
                </Text>
              )}
            </div>
            <div className="rounded-lg border border-ui-border-base p-4">
