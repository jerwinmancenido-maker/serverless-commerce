...
export default function ReminderPreferences({
  ...
  return (
    <form action={action} className="space-y-6 rounded-xl border border-ui-border-base bg-white p-5">
      ...
      <fieldset><legend className="text-sm font-medium">Research Hub schedules</legend><p className="mt-1 text-xs text-ui-fg-subtle">These choices control when Research Hub prompts are scheduled. Account Notification Settings controls whether optional event types appear.</p><div className="mt-3 grid grid-cols-1 gap-3 small:grid-cols-2">{[["daily_summary","Daily summary",preferences.daily_summary],["weekly_summary","Weekly schedule summary",preferences.weekly_summary],["replenishment_reminders","Replenishment reminders",preferences.replenishment_reminders],["progress_reminders","Progress measurement reminders",preferences.progress_reminders],["journal_prompts","Journal prompts",preferences.journal_prompts]].map(([name,label,checked]) => <label key={String(name)} className="flex items-center gap-2 text-sm"><input className={checkbox} type="checkbox" name={String(name)} defaultChecked={Boolean(checked)} />{label}</label>)}</div></fieldset>
