...
## PepStack project overlay

- Canonical product requirements are `docs/commerce-v1-spec.md`; canonical
  runtime boundaries are `docs/architecture.md`.
- Medusa is the only active commerce engine. Files under
  `legacy/next-drizzle-foundation` are migration reference and must not be
  wired back into runtime code.
- Payment providers and fulfillment/shipping options must remain configurable
  through Medusa. Do not hardcode Manual QR, J&T, voucher codes, rates, or
  credentials in the storefront.
- Do not run a database migration, seed, admin-user creation, deployment, or
  provider connection without explicit authorization for that external state.
